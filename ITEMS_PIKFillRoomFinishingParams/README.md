...
Плагин работает только для помещений, которые видны (выбираются) на виде. Рекомендуются запускать плагин при активной спецификации помещений,
в которой отфильтрованы только те помещения, для которых нужно выполнить расчет.
Плагин ищет ограждающие помещение стены, далее оставляет только те стены, у которых значение параметра “Группа модели” – Отделка.
Далее берет данные об отделке из типа стены и записывает в соответствующие параметры помещения наименование, площадь и длину из стен (ВАЖНО! длина вычисляется в мм)
Плагин ищет перекрытия и потолки, которые находятся в помещении или выше и ниже контура помещения на 92 мм (0.3 фута).
Оставляет только те перекрытия, у которых значение параметра “Группа модели” соответствует требованиям описанным ниже.
Далее берет данные об отделке из типа перекрытия и записывает в соответствующие параметры помещения наименование, площадь.
Для корректной работы плагина необходимо наличие требуемых парметров. Параметры типов стен и типов перекрытий обязательно нужно заполнить,
как указано в требования для работы плагина. Если по смыслу заполнять не нужно, оставляйте параметр пустым.

Ошибки:
В случае если не все требования работы будут выполнены, работа плагина не будет остановлена.
Возникающие ошибки будут записаны в параметр "Плагин_Ошибки в отделке" в экземпляр элемента (!!!ВАЖНО проверьте наличие параметра).
Для проверки правильности работы плагина, данные о помещении к которому относится элемент записываются в параметр "Плагин_Номер помещения".
...
Наличие заполненых параметров в типах Перекрытий и Потолков:
...
Группа модели - "перекрытие архитектурное", "Перекрытие архитектурное", "Потолок", "Пол"

Наличие параметров в экземплярах Перекрытий, Потолков, Дверей и Стен:
Плагин_Номер помещения (экземпляр, не зависит от группы, Текст)

Наличие параметров в экземплярах Помещений, Перекрытий, Потолков, Дверей и Стен:
Плагин_Ошибки в отделке (экземпляр, не зависит от группы, Текст)

!!!ВНИМАНИЕ Должен быть включен расчет объемов помещений:
Архитектура > Помещения и зоны ∨ Расчеты площадей и объемов > Расчеты > Площади и объемы ⊙
Элементы пола и потолка должны быть ограничены по площади помещения, не допускается пол и потолок сделанные одним элементом для нескольких помещений.
